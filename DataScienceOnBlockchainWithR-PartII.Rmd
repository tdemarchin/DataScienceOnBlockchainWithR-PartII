...
While both the transfer and the sales can be managed by the same contract, it is done differently in *OpenSea*. The sale is managed by the *OpenSea* contract and if it is approved (asked price reached), the main *OpenSea* contract calls the *Weird whales* contract which then triggers the transfer. If we want to know the price at which the NFTs were sold (in addition to the transfer discussed above), we need to extract data from this second contract. The sales are recorded by an event called *OrderMatch*.
...
```{r, fig.cap = "Figure 5: Timeline plot showing the frequence of the transations."}
plot(tEdgeFormation(dNetwork, time.interval = 5), ylab = "Frequence")
```
